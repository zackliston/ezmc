...
describe('dropIndexes', () => {
  let DB;
  let mongoDropIndexesMock;
  let mongoCollectionsMock;
  let mongoConnectMock;

  const dropIndexesOutput = Object.freeze({ fin: 'output' });
  ...
  beforeEach(() => {
    ...
    mongoDropIndexesMock = sinon.stub().returns(Promise.resolve(dropIndexesOutput));
    mongoCollectionsMock = sinon.stub().returns({
      dropIndexes: mongoDropIndexesMock
    });
  ...
  it('calls dropIndexes on the returned collection with the correct parameters', () => {
    ...
    return db.dropIndexes(collection)
    .then(() => {
      expect(mongoDropIndexesMock).to.have.been.calledWithMatch();
    });
  });
  ...
  it('returns the correct response', () => {
    ...
    return db.dropIndexes(collection)
    .then((response) => {
      expect(response).to.deep.equal(dropIndexesOutput);
    });
  });
});
